...
import { interactjs } from "./interact";
interactjs;
const socket: Socket<ServerToClientEvents, ClientToServerEvents> = io();
const devices = document.querySelector(".devices");
const panel = document.getElementById("panel");
let deviceData: SmartDevice[];
let isOpen = false;
socket.on("connect", () => {
...
const closePanel = () => {
  panel?.classList.add("panel__close");
  isOpen = false;
};
const openPanel = () => {
  panel?.classList.remove("panel__close");
  isOpen = true;
};

const createPanel = (data: any, type: deviceType) => {
  if (!panel) {
...
const createListElement = (el: SmartDevice) => {
  ...
  devices?.appendChild(wrapper);
};

const handleListClick = async (id: string, type: deviceType) => {
  if (!panel) {
    alert("Something went wrong!");
    return;
  }
  ...
  const result = await (await fetch("/api/v1/devices/" + id)).json();
...
const init = async () => {
  ...
  const width = localStorage.getItem("width") || String(panel?.scrollWidth);
  const height = localStorage.getItem("height") || String(panel?.scrollHeight);
  if (panel) {
  ...
  // fetch and render
  deviceData = (await (await fetch("/api/v1/devices")).json()) as SmartDevice[];
  deviceData.forEach((el) => {
    createListElement(el);
  });
  if (isOpen) openPanel();
